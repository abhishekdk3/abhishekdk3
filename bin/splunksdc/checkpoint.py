...
class Partition(object):
    ...
    def _strip(self, key):
        return key[self._prefix_size :]  # noqa
...
class LocalKVStore(object):
    ...
    def _replay(cls, fp):
        ...
        if (
            magic != b"BUK0"
        ):  # added b for python 2  and 3 compatability as recieving in bytes in python3
    ...
    def open_always(cls, filename):
        try:
            ...
            logger.error(
                "Open checkpoint failed. {exception}".format(exception=str(e)),
                path=filename,
            )
            exit()
