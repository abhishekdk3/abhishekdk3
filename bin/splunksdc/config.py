...
class ConfigManager(object):
    ...
    def load(self, name, stanza=None, virtual=False, clear_cred=False):
        path = self._make_path(name, stanza, virtual)
        content = self._cached_load(path, clear_cred)
        if stanza:
    ...
    def _cached_load(self, path, clear_cred=False):
        if path in self._cache:
            return self._cache[path]

        content = self._fresh_load(path, clear_cred)
    ...
    def _fresh_load(self, path, clear_cred=False):
        # This condition is added to resolve ADDON-24459
        if clear_cred:
            query = {"output_mode": "json", "count": 0, "--get-clear-credential--": "1"}
        else:
