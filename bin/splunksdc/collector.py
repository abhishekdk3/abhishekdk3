...
from splunklib.modularinput.scheme import ET, Scheme
...
from splunksdc.event_writer import HECWriter, XMLEventWriter
...
def _validate_definition(stream, scheme):
    ValidationDefinition.parse(stream)
    return 0
...
def run_modular_input(modular_input_factory, **kwargs):
    ...
    app = modular_input_factory(
        loop, app_name, modular_name, context, use_single_instance
    )
...
class SimpleCollectorTask(object):
    ...
    def start(self, name, params):
        ...
        worker = multiprocessing.Process(target=self._task_procedure, args=args)
        worker.daemon = True
        self._worker = worker
        self._worker.start()
...
class SimpleCollectorV1(object):
    ...
    def main(cls, delegate, **kwargs):
        def factory(loop, app_name, modular_name, context, use_single_instance):
            return cls(
                loop, app_name, modular_name, context, use_single_instance, delegate
            )

        run_modular_input(factory, **kwargs)

    def __init__(
        self, loop, app_name, modular_name, context, use_single_instance, delegate
    ):
